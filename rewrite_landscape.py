...
const LandscapeContactScreen = ({ onClose }: { onClose: () => void }) => {
      ...
      <div className="flex justify-between items-start w-full">
        <div className="relative">
          ...
          <motion.div 
            ...
            className="absolute top-[100%] left-[0.1em] mt-3 flex flex-col gap-2 pointer-events-auto select-none"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Card 1: Socials */}
            <div className="w-52 h-[3.25rem] bg-black rounded-lg flex items-center justify-center gap-6 border border-transparent hover:border-white/20 transition-colors">
              <a 
            ...
            <div 
              ...
              className="w-52 h-[3.25rem] bg-black rounded-lg flex items-center justify-between px-5 cursor-pointer border border-transparent hover:border-[#89CC04] transition-colors group"
            >
              <span className="font-sans text-white text-[15px] font-medium tracking-wide">Email cá nhân</span>
              <Copy strokeWidth={1.5} className="w-5 h-5 text-white group-hover:text-[#89CC04] transition-colors" />
            </div>
  ...
  return (
    ...
    >
      {/* Background container: We just want to fade in the color overlays, not the image itself, to make the transition smoother */}
      <div 
